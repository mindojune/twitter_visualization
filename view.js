...
class Chart {
  ...
  drawXAxis() {
    this.plot.append('g')
      .attr('id', 'x-axis')
      .attr('transform', `translate(0, ${this.plotHeight+10})`)
  ...
  drawBars() {
    ...
    bars
      .attr('transform', (d) => `translate(${this.xScale(d.key)+this.dotRadius}),5`);
    bars.enter()
      .append('g')
        .attr('class', 'bar')
        .attr('transform', (d) => `translate(${this.xScale(d.key)+this.dotRadius},5)`);
  ...
  static getDotCoordinates(index, chart) {
    let coordinates = {};
    coordinates.y = chart.height -50 - (chart.xScale.bandwidth() / chart.dotsPerBandWidth)
      * (Math.floor(index/chart.dotsPerBandWidth));
    coordinates.x = (chart.xScale.bandwidth() / chart.dotsPerBandWidth)
